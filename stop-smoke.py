...
# Descobre a linguagem do logon windows do usuário
...
trans = gettext.translation('stop-smoke', localedir=locale_dir, languages=[LANG])
trans.install()
...
class StopSmokingApp(tk.Frame):
    def __init__(self, master=None):
        ...
        self.master.title(gettext.translation("stop-smoke", localedir="locale", languages=[LANG]).gettext("Stop Smoking"))
    ...
    def create_widgets(self):
        ...
        self.time_left_label["text"] = gettext.translation("stop-smoke", localedir="locale", languages=[LANG]).gettext("Time left: {} minutes").format(int(max(0, self.last_smoke + self.wait_time - time.time()) / 60))
        self.time_left_label.pack(pady=15)

        # Cria um botão para registrar quando fumar
        self.smoke_button = tk.Button(self)
        self.smoke_button["text"] = gettext.translation("stop-smoke", localedir="locale", languages=[LANG]).gettext("Smoke")
    ...
    def update_screen(self):
        ...
        if self.get_time_left() > 0:
            ...
            self.time_left_label["text"] = gettext.translation("stop-smoke", localedir="locale", languages=[LANG]).gettext("You can smoke again")
